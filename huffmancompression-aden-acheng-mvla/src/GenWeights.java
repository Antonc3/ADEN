...
public class GenWeights extends Application {
	...
	private void generateWeights(String infName) {
		infName = "data/"+infName;
		String checkFile = checkFileNameRead(infName);
		if(checkFile.length() != 0) {
			Alert a = new Alert(AlertType.WARNING);
			a.setContentText(checkFile);
			a.show();
			return;
		}
	...
	private void saveWeightsToFile(String outfName) {
		...
		if(outfName.length()==0) {
			Alert a = new Alert(AlertType.WARNING);
			a.setContentText("Output file name cannot be blank");
			a.show();
			return;
		}
		outfName = "output/"+outfName;
		try {
			...
			if(!f.exists()) {
				...
				if(!f.canWrite()) {
					Alert a = new Alert(AlertType.WARNING);
					a.setContentText("Cannot write to file");
					a.show();
					return;
				}
			}
			...
			a.setContentText("Finished Writing to File");
			a.show();
		} catch(IOException e) {
			e.printStackTrace();
		}
	...
	private String checkFileNameRead(String fileName) {
		File f = new File(fileName);
		if(fileName.length() == 0) {
			return "File Name cannot be empty";
		}
		if(!f.isFile()) {
			return "given file name is not a file";
		}
		if(!f.canRead()) {
			return "Cannot Read Given File";
		}
		return "";
	}
	/**
